...
function setupEventListeners() {
    ...
    document.getElementById('checkout-form').addEventListener('submit', async (e) => {
        ...
        cart.forEach(item => {
            orderSummary += `- ${item.name} (${item.size}, ${item.color}) x ${item.quantity}: Rs. ${item.price * item.quantity}\n`;
        });
        orderSummary += `\nSubtotal: Rs. ${subtotal}`;
        if (discount > 0) orderSummary += `\nDiscount: - Rs. ${Math.round(discount)}`;
        orderSummary += `\nDelivery: Rs. 100`;
        orderSummary += `\nTOTAL: Rs. ${finalTotal}`;
        
        const detailsField = document.getElementById('checkout-order-details');
        if (detailsField) detailsField.value = orderSummary;
